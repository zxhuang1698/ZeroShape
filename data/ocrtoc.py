...
class Dataset(base.Dataset):
    ...
    def get_depth(self, idx):
        category, name = self.list[idx]
        depth_fname = "{}/depth_np/{}/{}.npy".format(self.path, category, name)
    ...
    def __getitem__(self, idx):
        ...
        # erode the mask for depth evaluation
        if self.opt.data.ocrtoc.erode_mask is not None:
            mask_eroded = self.erode_mask(mask, iterations=self.opt.data.ocrtoc.erode_mask)
            sample.update(mask_eroded=mask_eroded)
